...
def rules(env, notes=True):
    prompt = 'You are an agent in a cooperative game (Overcooked!). Your task is to work with other agents to achieve the goals of the game following the rules below.\n'
    ...
    if notes:
        ...
        prompt += 'When you are holding objects, you cannot get any more objects. If you want to remove the holding objects, you need to put them at the storage.\n'
...
def prepend_history(history, add, role='user', verbose=False):
    if verbose:
        print(f'[[{role}]]\n\n' + add)
